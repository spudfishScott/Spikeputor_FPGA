...
architecture RTL of VIDEO_WSH_P is
    ...
    signal timer        : integer range 0 to 10000000 := 0;                                         -- timer counter
                    ...
                    when INIT =>            -- go through the display reset and initialization sequence
                        ...
                        case cmd_index is
                            ...
                            when 114 =>      -- step 114: Delay 10 uS
                                ...
                                state <= WAIT_ST;
                            when 115 =>      -- step 115: Write 0x80 to Register 0x01 (Set up PLLs, TFT Output is 24 bpp)
                                d_in <= x"0080";
                                state <= DATA_WR;
                            ...
                            when 600 =>      -- step 600: Select Register 0x10
                                d_in <= x"0010";
                                state <= COMMAND_WR;
                            when 601 =>      -- step 601: Write 0x04 to Register 0x10 (Disable PIPs, 16 bpp main window)
                                d_in <= x"0004";
                                state <= DATA_WR;
                            ...
                            when 652 =>     -- step 652: Select Register 0x5E
                                d_in <= x"005E";
                                state <= COMMAND_WR;
                            when 653 =>     -- step 653: Write 0x01 to Register 0x5E (X-Y coordinate mode, 16 bpp active canvas/window)
                                d_in <= x"0001";
                                state <= DATA_WR;
                            when 654 =>     -- step 654: Select Register 0x10
                                d_in <= x"0010";
                                state <= COMMAND_WR;
                            when 655 =>     -- step 655: Write 0x04 to Register 0x10 (Disable PIPs, 16 bpp main window - final)
                                d_in <= x"0004";
                                state <= DATA_WR;
